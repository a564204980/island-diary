...
class _ElementSelectionBuilder extends StatefulWidget {
  ...
  @override
  _ElementSelectionBuilderState createState() =>
      _ElementSelectionBuilderState();
}
...
extension _ExportCanvasExtension on _DiaryBookExportPageState {
  Widget _buildCanvasElementWithCache(ExportElement element) {
    ...
    final h1 = Object.hash(
      ...
      element.cropRatio,
      element.textBackgroundColor?.toARGB32(),
    );
    final h2 = Object.hash(
      element.textBackgroundBorderRadius,
      element.textBackgroundOpacity,
      element.textBackgroundPadding,
      element.isLocked,
    );
          ...
          Builder(
            builder: (context) {
              return Positioned(
                ...
                child: CanvasBackgroundLayer(
                  key: ValueKey('canvas_bg_$i'),
                  imagePath: bg.imagePath,
                  color: bg.color,
                  width: _canvasWidth,
                  height: _canvasHeight,
                  cropRatio: bg.cropRatio,
                  bgX: bg.x,
                  bgY: bg.y,
                  bgScale: bg.scale,
        ...
        children: [
          ...
          ValueListenableBuilder<String?>(
            ...
            builder: (context, _, _) => Stack(
              children: [_buildSuspendedToolbar(), _buildDimensionBubble()],
            ),
          ),
        ],
      ),
  ...
  Widget _buildSuspendedToolbar() {
    if (_selectedElementId == null || _activeHandle != null) {
  ...
  Widget _buildDimensionBubble() {
    if (_activeHandle == null || _selectedElementId == null) {
      return const SizedBox.shrink();
    }
    final elementIdx = _elements.indexWhere((e) => e.id == _selectedElementId);
    if (elementIdx == -1) {
