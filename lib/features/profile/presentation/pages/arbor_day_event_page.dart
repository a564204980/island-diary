    ...
    return Stack(
      children: [
        Positioned.fill(
          child: CustomPaint(
            painter: _ForestSilhouettePainter(
              color: (isNight ? const Color(0xFF2E7D32) : const Color(0xFFC8E6C9)).withValues(alpha: 0.2),
              baseline: 0.6,
              seed: 123,
        ...
        Positioned.fill(
          child: CustomPaint(
            painter: _ForestSilhouettePainter(
              color: (isNight ? const Color(0xFF1B5E20) : const Color(0xFFA5D6A7)).withValues(alpha: 0.3),
              baseline: 0.75,
              seed: 456,
              isWideScreen: isWideScreen,
            ),
        ...
        Positioned.fill(
          child: CustomPaint(
            painter: _ForestSilhouettePainter(
              color: (isNight ? const Color(0xFF0D3214) : const Color(0xFF81C784)).withValues(alpha: 0.4),
              baseline: 0.9,
              seed: 789,
              isWideScreen: isWideScreen,
            ),
...
class _ForestSilhouettePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = minStep; x <= w; x += minStep + random.nextInt(randomStep.toInt()).toDouble()) {
    ...
    for (int i = 0; i < mountainPoints.length - 1; i++) {
      ...
      int treeCount = isWideScreen ? (1 + (random.nextDouble() > 0.7 ? 1 : 0)) : (random.nextDouble() > 0.2 ? 1 : 0);
  ...
  void _drawPine(Canvas canvas, Offset bottom, double height, Paint paint) {
    canvas.drawRect(Rect.fromLTWH(bottom.dx - 2, bottom.dy - height * 0.2, 4, height * 0.2), paint);
    final path = Path();
  ...
  void _drawRoundTree(Canvas canvas, Offset bottom, double height, Paint paint) {
    canvas.drawRect(Rect.fromLTWH(bottom.dx - 2, bottom.dy - height * 0.3, 4, height * 0.3), paint);
    double centerY = bottom.dy - height * 0.75;
    double radius = height * 0.35;
    canvas.drawCircle(Offset(bottom.dx, centerY), radius, paint);
    canvas.drawCircle(Offset(bottom.dx - radius * 0.6, centerY + radius * 0.2), radius * 0.7, paint);
    canvas.drawCircle(Offset(bottom.dx + radius * 0.6, centerY + radius * 0.2), radius * 0.7, paint);
    canvas.drawCircle(Offset(bottom.dx, centerY - radius * 0.4), radius * 0.8, paint);
  }

  void _drawBush(Canvas canvas, Offset bottom, double height, Paint paint) {
    double rx = height * 1.25;
    double ry = height;
    canvas.drawOval(Rect.fromCenter(center: Offset(bottom.dx, bottom.dy - ry), width: rx * 2, height: ry * 2), paint);
    canvas.drawOval(Rect.fromCenter(center: Offset(bottom.dx - rx * 0.5, bottom.dy - ry * 0.7), width: rx, height: ry), paint);
    canvas.drawOval(Rect.fromCenter(center: Offset(bottom.dx + rx * 0.5, bottom.dy - ry * 0.7), width: rx, height: ry), paint);
  }
      ...
      children: List.generate(15, (index) {
        ...
        return Positioned(
          ...
          child: Icon(
            ...
            color: (isNight ? Colors.green[300] : Colors.green[900])!.withValues(alpha: 0.12),
          )
          .animate(onPlay: (c) => c.repeat())
          ...
          .slideX(begin: 0, end: (random.nextDouble() - 0.5) * 2, duration: duration, curve: Curves.easeInOutSine),
        );
      }),
    );
  }
      ...
      children: [
        Positioned(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              boxShadow: [
                BoxShadow(color: Colors.yellow.withValues(alpha: 0.1), blurRadius: 30, spreadRadius: 5),
              ],
            ),
            child: Stack(
              children: [
                Container(decoration: const BoxDecoration(color: Color(0xFFFFE082), shape: BoxShape.circle)),
                Positioned(
                  left: 20,
                  top: -5,
                  child: Container(width: 75, height: 75, decoration: const BoxDecoration(color: Color(0xFF1B263B), shape: BoxShape.circle)),
                ),
              ],
            ),
          ).animate(onPlay: (c) => c.repeat(reverse: true)).moveY(begin: 0, end: -10, duration: 4.seconds, curve: Curves.easeInOut),
        ),
        ...List.generate(20, (index) {
          ...
          return Positioned(
            ...
            child: Container(width: size, height: size, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle))
            .animate(onPlay: (c) => c.repeat(reverse: true))
            .scale(begin: const Offset(0.4, 0.4), end: const Offset(1.2, 1.2), duration: (1.5 + random.nextDouble() * 2.5).seconds, delay: (random.nextDouble() * 2).seconds)
