...
class _DiaryDetailPageState extends State<DiaryDetailPage> {
  ...
  List<Widget> _buildBlocksView() {
    ...
    if (_currentEntry.isMixedLayout) {
      ...
      displayBlocks.addAll(processedBlocks);
    }
    ...
    for (var block in displayBlocks) {
      if (block is TextWrapGroupBlock) {
        list.add(_buildTextWrapGroupItem(block, textStyle));
        isFirst = false;
        // 增进 textBlockIndex 计数
        textBlockIndex++;
  ...
  Map<String, dynamic> _splitTextSpan({
    ...
    if (shouldNotSplit) {
      final span = tc.buildTextSpan(
        ...
        hideMarkdownSymbols: true,
        annotations: _currentEntry.annotations,
        blockIndex: _currentEntry.blocks.indexWhere((b) => b['id'] == textBlock.id),
        onAnnotationTap: (key) => _showAnnotationSheet(key: key),
      );
    ...
    final narrowCtrl = DiaryTextEditingController(text: narrowText, attributes: narrowAttrs);
    narrowCtrl.baseColor = textStyle.color ?? Colors.black;
    narrowCtrl.baseFontFamily = textStyle.fontFamily ?? 'LXGWWenKai';
    narrowCtrl.baseFontSize = textStyle.fontSize ?? 20;

    final remainingCtrl = DiaryTextEditingController(text: remainingText, attributes: remainingAttrs);
    remainingCtrl.baseColor = textStyle.color ?? Colors.black;
    remainingCtrl.baseFontFamily = textStyle.fontFamily ?? 'LXGWWenKai';
    remainingCtrl.baseFontSize = textStyle.fontSize ?? 20;

    final narrowSpan = narrowCtrl.buildTextSpan(
      context: context,
      style: textStyle,
      withComposing: false,
      hideMarkdownSymbols: true,
    ...
    final remainingSpan = remainingCtrl.buildTextSpan(
      context: context,
      style: textStyle,
      withComposing: false,
      hideMarkdownSymbols: true,
  ...
  Widget _buildTextWrapGroupItem(TextWrapGroupBlock wrapBlock, TextStyle textStyle) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final splitResult = _splitTextSpan(
          textBlock: textBlock,
          context: context,
          textStyle: textStyle,
        ...
        final int textIndex = _currentEntry.blocks.indexWhere((b) => b['id'] == textBlock.id);
          ...
          children: [
              ...
              children: [
                if (alignment == 'left') ...[
                  SizedBox(
                    ...
                    child: DiaryBlockItem(
                      key: ValueKey(imageBlock.id),
                      block: imageBlock,
                      index: _currentEntry.blocks.indexWhere((b) => b['id'] == imageBlock.id),
                      isFloatingOverride: true,
                      floatingHeight: narrowTextHeight - 16,
                      isNightOverride: widget.isNight,
                      paperStyle: _currentEntry.paperStyle,
                      accentColor: accentColor,
                      onShowPreview: (img) => _showImagePreviewDialog(img.file.path, img.id),
                    ),
                  ),
                  ...
                  child: Stack(
                    ...
                    children: [
                      ValueListenableBuilder<TextEditingValue>(
                        valueListenable: ValueNotifier(TextEditingValue(text: narrowSpan.toPlainText())),
                        builder: (builderContext, value, child) {
                          final narrowCtrl = DiaryTextEditingController(
                            text: narrowSpan.toPlainText(),
                            attributes: (textBlock.controller as DiaryTextEditingController).attributes
                                .where((a) => a.start < narrowSpan.toPlainText().length)
                                .toList(),
                          );
                          narrowCtrl.baseColor = textStyle.color ?? Colors.black;
                          narrowCtrl.baseFontFamily = textStyle.fontFamily ?? 'LXGWWenKai';
            ...
            if (remainingSpan != null && remainingText.isNotEmpty) ...[
              ...
              Stack(
                ...
                children: [
                  ValueListenableBuilder<TextEditingValue>(
                    valueListenable: ValueNotifier(TextEditingValue(text: remainingSpan.toPlainText())),
                    builder: (builderContext, value, child) {
                      final remText = remainingSpan.toPlainText();
                      final remainingCtrl = DiaryTextEditingController(
                        text: remText,
                        attributes: (textBlock.controller as DiaryTextEditingController).attributes
                            .where((a) => a.end > trimStartOffset)
                            .map((a) => TextAttribute(
                                  start: (a.start - trimStartOffset).clamp(0, remText.length),
                                  end: (a.end - trimStartOffset).clamp(0, remText.length),
                                  color: a.color,
